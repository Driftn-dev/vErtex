...
class vErtexEnterprise:
    ...
    def web_application_analysis(self):
        ...
        try:
            ...
            # Sanity check: if the response carries suspiciously few headers,
            # it's likely a CDN bot-challenge / reduced response rather than
            # the real page — flag it instead of silently reporting every
            # security header as "Missing".
            if len(resp.headers) < 5:
                self.log(
                    "WEB",
                    f"Response had only {len(resp.headers)} headers — possible "
                    f"bot-detection/CDN challenge page rather than the real site. "
                    f"Header results below may be unreliable.",
                    "MEDIUM", 0
                )
